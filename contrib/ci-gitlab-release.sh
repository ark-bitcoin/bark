...
if [ -z "${CI_PROJECT_NAME:-}" ]; then
  echo "Error: CI_PROJECT_NAME is not set" >&2
  exit 1
fi
...
glab release create "${CI_COMMIT_TAG}" ${FILE_NAMES} \
	--repo "ark-bitcoin/${CI_PROJECT_NAME}" \
	--use-package-registry \
	--notes "Automated release ${CI_COMMIT_TAG} from GitLab CI"

echo "GitLab release: https://gitlab.com/ark-bitcoin/${CI_PROJECT_NAME}/-/releases/${CI_COMMIT_TAG}"
