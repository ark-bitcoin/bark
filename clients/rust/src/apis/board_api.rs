...
/// struct for typed errors of method [`board`]
...
pub async fn board(configuration: &configuration::Configuration, board_request: models::BoardRequest) -> Result<models::Board, Error<BoardError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_board_request = board_request;

    let uri_str = format!("{}/api/v1/board/board", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<BoardError> = serde_json::from_str(&content).ok();
...
pub async fn board_all(configuration: &configuration::Configuration, ) -> Result<models::Board, Error<BoardAllError>> {
    ...
    let uri_str = format!("{}/api/v1/board/board/all", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
