/*
 * Barkd API
 *
 * A simple REST API for Barkd
 *
 * The version of the OpenAPI document: 0.1.0-beta.4
 * Contact: hello@second.tech
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// VtxoInfo : Struct representing information about a VTXO.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct VtxoInfo {
    #[serde(rename = "amount_sat")]
    pub amount_sat: i64,
    #[serde(rename = "chain_anchor")]
    pub chain_anchor: String,
    #[serde(rename = "exit_delta")]
    pub exit_delta: i32,
    #[serde(rename = "exit_depth")]
    pub exit_depth: i32,
    #[serde(rename = "expiry_height")]
    pub expiry_height: i32,
    #[serde(rename = "id")]
    pub id: String,
    #[serde(rename = "policy_type")]
    pub policy_type: String,
    #[serde(rename = "server_pubkey")]
    pub server_pubkey: String,
    #[serde(rename = "user_pubkey")]
    pub user_pubkey: String,
}

impl VtxoInfo {
    /// Struct representing information about a VTXO.
    pub fn new(amount_sat: i64, chain_anchor: String, exit_delta: i32, exit_depth: i32, expiry_height: i32, id: String, policy_type: String, server_pubkey: String, user_pubkey: String) -> VtxoInfo {
        VtxoInfo {
            amount_sat,
            chain_anchor,
            exit_delta,
            exit_depth,
            expiry_height,
            id,
            policy_type,
            server_pubkey,
            user_pubkey,
        }
    }
}

