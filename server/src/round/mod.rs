...
use ark::vtxo::Full;
use ark::{
	ProtocolEncoding, SignedVtxoRequest, Vtxo, VtxoId, VtxoPolicy, VtxoRequest,
};
...
pub struct SelfSignedInput {
	pub vtxo_id: VtxoId,
	pub attestation: RoundAttemptAttestation,
}

#[derive(Debug)]
pub struct DelegatedInput {
	pub vtxo_id: VtxoId,
	pub attestation: DelegatedRoundParticipationAttestation,
}

#[derive(Debug)]
pub enum RoundInput {
	RegisterPayment {
		inputs: Vec<SelfSignedInput>,
		...
		/// Bucketed user agent name captured by the RPC middleware at
...
pub struct InteractiveParticipation {
	...
	/// Bucketed user agent name from the SubmitPayment RPC that produced
	/// this participation. Used to attribute `refresh_counter` /
	/// `refresh_volume` per-integrator when the round finalizes.
	pub(crate) user_agent_name: &'static str,
	/// Refresh fee paid by the user for this participation (input surplus
	/// over outputs), captured at registration and recorded as fee telemetry
	/// once the round succeeds.
	pub(crate) user_fee_sat: u64,
}
...
pub struct DelegatedParticipation {
	...
	/// `user_agent` column on that table we can't recover the originator here,
...
pub struct CollectingPayments {
	...
	round_attempt_challenge: Challenge,
...
impl CollectingPayments {
	fn new(
		...
		CollectingPayments {
			round_data,
			round_attempt_challenge: Challenge::generate(),
			locked_inputs,
			allowed_inputs,
	...
	fn validate_payment_data(
		...
		for input in inputs {
			if !unique_inputs.insert(input) {
				client_rslog!(RoundUserVtxoDuplicateInput, self.round_step, vtxo: input.vtxo_id);
				bail!("user provided duplicate inputs");
			}
			if self.all_inputs.contains_key(&input.vtxo_id) {
				client_rslog!(RoundUserVtxoAlreadyRegistered, self.round_step, vtxo: input.vtxo_id);
		...
		if let Some(ref allowed) = self.allowed_inputs {
			...
			if let Some(bad) = inputs.iter().find(|i| !allowed.contains(&i.vtxo_id)) {
				client_rslog!(RoundUserVtxoNotAllowed, self.round_step, vtxo: bad.vtxo_id);
	...
	pub async fn process_self_signed_participation(
		&mut self,
		srv: &Server,
		inputs: Vec<SelfSignedInput>,
		vtxo_requests: Vec<SignedVtxoRequest>,
		...
		let input_vtxos = match self.check_fetch_round_input_vtxos(srv, &input_ids).await {
			...
			Err(e) => {
				let ret = if let Some(id) = e.downcast_ref::<VtxoId>().cloned() {
					client_rslog!(RoundUserVtxoUnknown, self.round_step, vtxo: Some(id));
		...
		let attestation_by_vtxo_id = inputs.iter()
			.map(|v| (v.vtxo_id, &v.attestation))
			.collect::<HashMap<_,_>>();
		for input in &input_vtxos {
			let attestation = attestation_by_vtxo_id.get(&input.id())
				.expect("all vtxos were found");
			attestation.verify(self.round_attempt_challenge, input, &vtxo_requests)
				.with_context(|| format!(
					"attestation is invalid: vtxo {}, proof: {:?}", input.id(), attestation,
				))?;
		}
		...
		let out_sum = vtxo_requests.iter().map(|r| r.as_ref().amount).sum::<Amount>();
		let actual_fee = in_sum.checked_sub(out_sum)
			.expect("checked by validate_payment_amounts");

		// Finally, we are done
		self.register_interactive_participation(
			flux_guard, input_vtxos, vtxo_requests, unlock_preimage, user_agent_name,
			actual_fee.to_sat(),
		);
	...
	async fn process_delegated_participation(
		...
		// Fee is not re-checked here on purpose: the delegated participation
		// was already validated with pver-aware `validate_refresh_fee` in
		// `register_delegated_round_participation`, and its stored invariant
		// (fees only decrease as the tip advances) means it still covers the
		// round-time fee. The actual fee is still captured for telemetry
		// parity with the interactive path.
		let in_sum = input_vtxos.iter().map(|v| v.amount()).sum::<Amount>();
		let out_sum = participation.outputs.iter()
			.map(|o| o.vtxo_request.amount).sum::<Amount>();
		let actual_fee = in_sum.checked_sub(out_sum)
			.expect("checked by validate_payment_amounts in register_delegated_round_participation");
...
impl SigningVtxoTree {
	...
	async fn finish(mut self, srv: &Server) -> Result<RoundFinished, RoundError> {
		...
		// as add_round). refresh_volume + delegated_participation_volume ==
		// round_volume. Refresh fees are recorded here too — this block runs
		// exactly once per successful round, so no double-count on retries.
		let round_seq = self.round_step.round_seq();
		for participation in self.interactive_participants.values() {
			let vol = participation.inputs.iter()
				...
				.map(|v| v.amount())
				.sum::<Amount>();
			telemetry::add_refresh(vol.to_sat(), participation.user_agent_name);
			telemetry::record_ark_fee(
				telemetry::ArkFeeOp::Refresh, participation.user_fee_sat, Some(round_seq),
			);
		}
		for participation in self.delegated_participants.values() {
			let vol = participation.inputs.iter()
				...
				.map(|v| v.amount())
				.sum::<Amount>();
			telemetry::add_delegated_participation(vol.to_sat(), participation.user_agent_name);
			telemetry::record_ark_fee(
				telemetry::ArkFeeOp::Refresh, participation.user_fee_sat, Some(round_seq),
			);
		}
		...
		for output in &self.real_outputs {
			...
			match output.policy {
				...
				VtxoPolicy::ServerHtlcRecv { .. } | VtxoPolicy::ServerHtlcRecv_v0 { .. } => {
					telemetry::add_round_output_htlc_recv(amount_sats);
				},
			}
		}
			...
			input = round_input_rx.recv() => {
				...
				let res = match input {
					RoundInput::RegisterPayment {
						inputs, vtxo_requests, unlock_preimage, user_agent_name
					} => {
						state
							.process_self_signed_participation(
								srv, inputs, vtxo_requests, unlock_preimage, user_agent_name,
							).await
							.map_err(|e| {
...
#[tracing::instrument(
	...
	fields(
		{ telemetry::ATTRIBUTE_ROUND_SEQ } = %round_seq,
		{ telemetry::ATTRIBUTE_ATTEMPT_SEQ } = tracing::field::Empty,
		challenge = tracing::field::Empty,
	)
)]
async fn perform_round_attempt(
	srv: &Server,
	round_input_rx: &mut mpsc::UnboundedReceiver<(RoundInput, oneshot::Sender<anyhow::Error>)>,
	round_seq: RoundSeq,
	mut round_state: RoundState,
) -> RoundAttemptResult {
	let state = round_state.collecting_payments();
	...
	RoundAttemptResult::Finished(round_state.result().unwrap())
}
...
mod tests {
	...
	lazy_static::lazy_static! {
		static ref TEST_ATTESTATION: RoundAttemptAttestation = RoundAttemptAttestation::deserialize_hex(
	...
	fn test_register_payment_valid() {
		...
		state.register_interactive_participation(flux.empty_guard(), inputs, outputs.clone(), pre, "test", 0);
	...
	// The fee check has no rounding tolerance: paying the exact expected fee is
	// accepted and even a one sat shortfall is rejected.
	#[test]
	fn test_register_payment_rejects_one_sat_fee_shortfall() {
		let state = create_collecting_payments(2);
		...
		let fees = RefreshFees {
			base_fee: Amount::from_sat(100),
			ppm_expiry_table: vec![
				PpmExpiryFeeEntry { expiry_blocks_threshold: 0, ppm: PpmFeeRate(500) },
				PpmExpiryFeeEntry { expiry_blocks_threshold: 50_505, ppm: PpmFeeRate(1_000) },
			],
		};
	...
	fn test_refresh_fee_at_scheduled_height() {
		...
		let outputs = vec![create_signed_req(output_amount.to_sat(), &state.round_data)];
		validate_refresh_fee(
			&inputs, &outputs, &fees, current_height,
		).unwrap_err();
		validate_refresh_fee(
			&inputs, &outputs, &fees, scheduled_height,
		).unwrap();
	}
	...
	fn test_register_payment_duplicate_cosign_pubkey() {
		...
		state.register_interactive_participation(flux.empty_guard(), inputs1, outputs1, pre, "test", 0);
	...
	fn test_register_multiple_payments() {
		...
		let flux = VtxosInFlux::new();
		state.validate_payment_data(&input_ids1, &outputs1).unwrap();
		state.register_interactive_participation(flux.empty_guard(), inputs1, outputs1.clone(), pre1, "test", 0);
		state.validate_payment_data(&input_ids2, &outputs2).unwrap();
		state.register_interactive_participation(flux.empty_guard(), inputs2, outputs2.clone(), pre2, "test", 0);
