...
mod test {
	...
	fn forfeit_state_encoding() {
		...
		let ffs = ForfeitState {
			...
			sec_nonces: vec![
				Secret::new(musig::DangerousSecretNonce::dangerous_from_secret_nonce(secn)),
			],
		};
		...
		let decoded = rmp_serde::from_slice::<ForfeitState>(&encoded).unwrap();
		assert_eq!(stable, decoded);
	}
}
