...
**pending_exit_sat** | Option<**i64**> | Sats held in VTXOs whose unilateral exit chain is confirmed on-chain but which haven't yet been drained to the onchain wallet. Equivalent to the sum of `Exited` VTXOs whose exit state hasn't reached `Claimed`. `null` if the exit subsystem is unavailable. | [optional]
