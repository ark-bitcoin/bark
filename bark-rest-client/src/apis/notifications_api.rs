...
/// struct for typed errors of method [`wait_notification`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum WaitNotificationError {
...
pub async fn wait_notification(configuration: &configuration::Configuration, since: Option<String>) -> Result<models::WaitNotificationResponse, Error<WaitNotificationError>> {
    ...
    let uri_str = format!("{}/api/v1/notifications/wait", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::WaitNotificationResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::WaitNotificationResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<WaitNotificationError> = serde_json::from_str(&content).ok();
