...
/// struct for typed errors of method [`offboard_fee`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum OffboardFeeError {
    Status400(models::BadRequestError),
...
pub async fn offboard_fee(configuration: &configuration::Configuration, offboard_fee_estimate_request: models::OffboardFeeEstimateRequest) -> Result<models::FeeEstimateResponse, Error<OffboardFeeError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_offboard_fee_estimate_request = offboard_fee_estimate_request;

    let uri_str = format!("{}/api/v1/fees/offboard", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_offboard_fee_estimate_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<OffboardFeeError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
