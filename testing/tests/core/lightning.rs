...
async fn cln_can_pay_lightning() {
	...
	info!("Connect `{}` to `{}`", lightning.internal.name, lightning.external.name);
	let mut grpc_client = lightning.internal.grpc_client().await;
	let peers = grpc_client.list_peers(rpc::ListpeersRequest{
		id: Some(lightning.external.id().await),
		level: None
	...
	ctx.fund_lightning(&lightning.internal, btc(5)).await;
	ctx.generate_blocks(6).await;
	lightning.internal.wait_for_block_sync().await;


	info!("Lightningd_1 opens channel to lightningd_2");
	// Open a channel from lightningd_1 to lightningd_2
	lightning.internal.fund_channel(&lightning.external, btc(1)).await;
	lightning.internal.bitcoind().generate(6).await;
	lightning.internal.wait_for_block_sync().await;
	...
	let invoice = lightning.external.invoice(Some(sat(1000)), "test_label", "Test Description").await;
	trace!("sender node pays the invoice");
	lightning.internal.pay_bolt11(invoice).await;
	lightning.external.wait_invoice_paid("test_label").await;
}
